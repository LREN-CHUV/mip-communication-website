...
const mockNews = [
  {
    ...
    content: 'the MIP is excited to announce the subproject\'s upcoming Workshop that will take'
        ...
        {[mockNews.map(news => (
          <li className="item">
            <article className="column-item">
              <h3 className="bullet bullet-bg-orange">{news.title}</h3>
              <div className="article-sub-text">{news.date}</div>
              <img className="article-image" src={news.image} alt="placeholder" />
              <p>{news.content}</p>
              <Link to={`/article/${news.id}`} className="read-more align-right clear">Read more</Link>
            </article>
          </li>
        ))]}
      </ul>
